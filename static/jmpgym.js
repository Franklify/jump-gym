...
scorePos = 0,
score1Pos = 0,
scoreTens = initialTens,
score1Tens = initialTens,
calories = 0,
calories1 = 0,
record = 0,
newRecord = 0,
...
// Audio assets
jumpGym = new Audio("JumpGym.mp3"),
snd = new Audio("jumping_teon.mp3"),
snd1 = new Audio("jumping_teon1.mp3"),
punch = new Audio("punch_sound.mp3"),
punch1 = new Audio("punch_sound1.mp3"),
swamp = new Audio("swamp_sounds.mp3"),
banjo = new Audio("banjo.mp3"),
banjo_start = new Audio("banjo_start.mp3"),
banjo_count = new Audio("banjo_count.mp3"),
banjo_end = new Audio("banjo_end.wav"),
        ...
        for(var i = 0, len = this._pipes.length; i < len; i++){
            ...
            if(i == 0){
                if (bird.dead == true && bird1.dead == true){
                    ...
                    banjo_end.play();
                    canGoToMenu = false;
                    setTimeout(function() {canGoToMenu = true;}, scoreResetTime);
                ...
                if (p.x <= bird.x && p.cleared == 1){
                    if (bird.y + .5 *  (s_bird[1].height) > (p.y + (1 - logScaler[p.index]) * s_pipe[p.index].height)){
                        ...
                        score++;
                        if (score % scoreTens == 0) {
                            scoreTens = 10 * scoreTens;
                            scorePos++;
                        }
                        p.cleared = 2;
                    }
                ...
                if (p.x <= bird1.x && p.cleared1 == 1){
                    if (bird1.y + .5 * (s_bird[1].height) > (p.y + (1 - logScaler[p.index]) * s_pipe[p.index].height)){
                        ...
                        score1++;
                        if (score1 % score1Tens == 0) {
                            score1Tens = 10 * score1Tens;
                            score1Pos++;
                        }
                        p.cleared1 = 2;
                    }
...
function onpress(evt){
    ...
    switch(currentstate){
        ...
        case states.Score:
            if ((evt.keyCode == 32 || evt.keyCode == 37 || evt.keyCode == 38 || evt.keyCode == 39) && canGoToMenu)
            {   
                resetter();
                bird.canJump = false;
                setTimeout(function(){bird.canJump = true;}, menuResetTime);
...
function update(){
    ...
    else if(bird1.dead){ // If current state is Score
        setTimeout(function(){bird.canJump = true;}, menuResetTime);
...
function render(){
    ...
    { 
        ...
        if (DisplayCounters === true){
            if(SinglePlayer === false){
                s_counters.draw(ctx, width2 - s_counters.width / 2 + counter_separation, 0, .7, .7);
                s_numberS.draw(ctx, width2 + counter_separation - 12 * score1Pos, counter_height + 62, score1);                
            }
            else {
                s_counters.draw(ctx, width2 - s_counters.width / 2 + counter_separation, 0, .7, .7);
            }
            s_numberS.draw(ctx, width2 + counter_separation - 12 * scorePos, counter_height + 32, score);
            s_numberS.draw(ctx, width2 - counter_separation - 12 * timerPos, counter_height + 32, timerCount);           
        }
    }
}
